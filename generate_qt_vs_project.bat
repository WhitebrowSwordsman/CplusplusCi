...
set vs_qt_dir=%bat_dir%\build\vs2022\CplusplusCi
cmake -S %bat_dir% -B "%vs_qt_dir%" -G "Visual Studio 17 2022" -A x64 -T v143 -DWINDOWS:BOOL=ON -DCMAKE_PREFIX_PATH=%qt_msvc_path% -DQT_DLL_DIR="%qt_msvc_path%\bin"
set cmake_generate_err=%ERRORLEVEL%
if %cmake_generate_err% NEQ 0 (
  exit /b %cmake_generate_err%
)
set qt_app=%bat_dir%\CplusplusCi.sln
if exist %qt_app% (
  echo '%qt_app%' exist
) else (
  mklink "%qt_app%" "%vs_qt_dir%\CplusplusCi.sln"
)
